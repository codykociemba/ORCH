...
export class WikiService {
  ...
  async status(opts: { probeRemote?: boolean } = {}): Promise<WikiStatus> {
    ...
    const detection = detectWikiProvider({
      origin,
      remotes,
      configured: workflow?.wiki?.provider,
  ...
  async evidence(mode: WikiEvidence['mode']): Promise<WikiEvidence> {
    ...
    return {
      ...
      index_current: true,
      status: status.bootstrap_required ? 'bootstrap_required' : (status.pages_generated > 0 ? 'passed' : 'skipped'),
      pages_generated: status.pages_generated,
      summary: status.bootstrap_required
        ? 'Remote publication: one-time GitHub wiki bootstrap required'
        : undefined,
    };
  }
  ...
  async generate(args: string[] = []): Promise<WikiRunResult> {
    const workflow = await new WorkflowConfigStore(this.projectRoot).read();
    return this.runGitnexusWiki([...wikiGenerateArgs(workflow), ...args]);
  }
  ...
  async publish(opts: { force?: boolean; trustedDefaultBranch?: boolean } = {}): Promise<WikiRunResult> {
    ...
    if (status.bootstrap_required) {
      return {
        stdout: [
          'BOOTSTRAP_REQUIRED',
          'Enable the repository Wiki if needed and create the first page once in GitHub.',
  ...
  private async publishGitHub(status: WikiStatus): Promise<number> {
    ...
    const remote = githubWikiRemote(owner, repo);
    ...
    const plan = planWikiSync({ generated, remote: remotePages, sha });
  ...
  private async publishGitLab(status: WikiStatus): Promise<number> {
    const token = process.env['ORCH_GITLAB_WIKI_TOKEN'] ?? process.env['GITLAB_TOKEN'] ?? process.env['CI_JOB_TOKEN'] ?? '';
    if (!token) {
      throw new WikiPublishError(
    ...
    const base = `${gitlabWikiApiBase()}/projects/${project}/wikis`;
    ...
    const listed = await this.http(base, { headers });
    if (!listed.ok) {
      throw new WikiPublishError(
    ...
    const plan = planWikiSync({ generated, remote, sha });
    for (const page of plan.write) {
    ...
    for (const name of plan.remove) {
      ...
      const response = await this.http(`${base}/${encodeURIComponent(slug)}`, { method: 'DELETE', headers });
  ...
  private async runGitnexusWiki(args: string[]): Promise<WikiRunResult> {
    try {
...
function wikiGenerateArgs(workflow: WorkflowConfig | null): string[] {
  const args: string[] = [];
  const llm = workflow?.wiki?.generator?.llm;
  const provider = process.env['GITNEXUS_WIKI_PROVIDER'] ?? (llm?.provider && llm.provider !== 'auto' ? llm.provider : undefined);
  const key = process.env['GITNEXUS_WIKI_API_KEY'];
  const model = process.env['GITNEXUS_WIKI_MODEL'] ?? llm?.model;
  const language = workflow?.wiki?.generator?.language;
  if (provider) args.push('--provider', provider);
  if (key) args.push('--api-key', key);
  if (model) args.push('--model', model);
  if (language) args.push('--lang', language);
  if (process.env['GITNEXUS_WIKI_FORCE'] === '1' || workflow?.wiki?.generator?.force) args.push('--force');
  return args;
}
